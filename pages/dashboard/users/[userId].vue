...
const userAdminStore = useAdminUsersStore();
...
onMounted(async () => {
  ...
  if (userId) {
    await userAdminStore.fetchUserDetails(userId);
    user.value = userAdminStore.user; // Ensure the user data is set after fetching details
  } else {
    logger.warn("No user ID provided in route");
  }
});
</script>
