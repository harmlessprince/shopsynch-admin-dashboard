...
const EMPTY_FORM_ERRORS = { attributeValue: '', skuError: '' }
const formErrors = reactive({ ...EMPTY_FORM_ERRORS })
...
watch(() => props.open, async (open) => {
  if (!open) return
  Object.assign(formErrors, EMPTY_FORM_ERRORS)
...
async function handleImageUpload(event) {
  ...
  try {
    ...
    fd.append('file', file)
    if (props.product?.tenantId) {
      fd.append('tenantId', props.product.tenantId)
    }
...
async function save() {
  ...
  if (!primaryDim.value) {
    // blocking state already shown inline — no toast needed
    return
  }

  if (!group.attributeValue.trim()) {
    formErrors.attributeValue = `${attributeLabel.value} is required.`
    return
  }

  if (selectedOptionValues.value.has(group.attributeValue.trim())) {
    formErrors.attributeValue = `A variant with the value '${group.attributeValue.trim()}' already exists on this product.`
    return
  }
  ...
  for (const variantGroup of groupsToValidate) {
    for (const sku of variantGroup.skus || []) {
      ...
      if (missingKey) {
        ...
        formErrors.skuError = `'${variantGroup.attributeValue}' has a SKU with no ${dim?.label || missingKey} value. Fill in ${dim?.label || missingKey} for every SKU before saving.`
        return
      }
    }
  }
  ...
  <Drawer :open="open" content-class="bg-white p-0 w-full" @update:open="$emit('update:open', $event)">
    <div class="flex flex-col h-full bg-white">
      ...
      <div class="flex items-center justify-between p-6 border-b border-gray-100 bg-slate-50/50">
        ...
        <button
          type="button"
          class="p-2 hover:bg-white rounded-full transition-colors text-slate-400 hover:text-rose-500 shadow-sm border border-transparent hover:border-slate-100 focus:ring-2 focus:ring-[#003366] focus:outline-none"
        ...
        >
          ...
          <div v-if="!primaryDim" class="flex items-start gap-2 p-4 border border-rose-200 rounded-xl bg-rose-50">
            <span class="material-symbols-outlined text-rose-500 text-base mt-0.5 shrink-0" aria-hidden="true">error</span>
            <p class="text-sm text-rose-700">
              No variation attributes are configured for this product's category. Ask your admin to add variation attributes (like Color or Size) to the category template before adding new variants.
            </p>
          </div>
          ...
          <div class="space-y-3">
            ...
            >
              ...
              <template v-else>
                <span class="material-symbols-outlined text-slate-300 text-3xl mb-1" aria-hidden="true">add_a_photo</span>
              ...
              <div v-if="primaryOptions.length > 0" class="space-y-2">
                ...
                <div class="flex flex-wrap gap-2">
                  <button
                    ...
                    @click="group.attributeValue = option; formErrors.attributeValue = ''"
                  >
                    {{ option }}
                  </button>
                </div>
                <p v-if="formErrors.attributeValue" class="text-xs text-rose-600 font-medium mt-1">{{ formErrors.attributeValue }}</p>
              </div>
              <AppInput
                v-else
                ...
                @update:model-value="formErrors.attributeValue = ''"
              />
            </div>
	        </div>
        ...
        <!-- SKU tiers -->
        <div class="space-y-6">
          <div v-if="formErrors.skuError" class="flex items-start gap-2 px-3 py-2.5 bg-rose-50 border border-rose-200 rounded-lg">
            <span class="material-symbols-outlined text-rose-500 text-base mt-0.5 shrink-0" aria-hidden="true">error</span>
            <p class="text-xs text-rose-700 font-medium">{{ formErrors.skuError }}</p>
          </div>
          <div class="flex items-center justify-between">
            ...
            <button
              type="button"
              class="px-4 py-2 bg-primary/5 text-primary hover:bg-primary/10 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all focus:ring-2 focus:ring-[#003366] focus:outline-none"
              @click="addSku"
            >
              + Add SKU
            </button>
          ...
          <div class="space-y-6">
            ...
            >
              <button
                ...
                type="button"
                class="absolute -right-3 -top-3 w-8 h-8 bg-rose-500 text-white rounded-full flex items-center justify-center shadow-lg hover:bg-rose-600 transition-colors z-10 focus:ring-2 focus:ring-[#003366] focus:outline-none"
      ...
      <div class="p-8 border-t border-gray-100 bg-slate-50/50 flex gap-4 justify-end items-center">
        ...
        <BaseButton variant="outline" class="min-w-[140px]" @click="close">Cancel</BaseButton>
	        <BaseButton :loading="isSubmitting" :disabled="!primaryDim" class="min-w-[140px]" @click="save">Create Variant</BaseButton>
      </div>
    </div>
  </Drawer>
</template>
