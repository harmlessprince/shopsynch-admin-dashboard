...
const handleFileUpload = async (event, index) => {
  ...
  try {
    ...
    if (props.product?.tenantId) {
      formData.append('tenantId', props.product.tenantId)
    }
...
<template>
  ...
  >
    <div class="flex items-center justify-between p-8 border-b border-gray-50 bg-slate-50/50">
      ...
      <button class="p-3 hover:bg-white rounded-full transition-all text-slate-400 hover:text-rose-500 shadow-sm" @click="close">
    ...
    <div class="flex-1 overflow-y-auto p-10">
        ...
        >
          ...
          <template v-if="img">
            ...
            <div class="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-all flex flex-col items-center justify-center gap-4 backdrop-blur-[2px]">
              ...
              <button class="px-6 py-2 bg-rose-500/20 text-rose-500 hover:bg-rose-500 hover:text-white border border-rose-500/30 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all" @click="removeImage(index)">
                Remove
              </button>
            </div>
      ...
      <div v-if="imageList.length === 0" class="py-20 flex flex-col items-center justify-center text-center">
         ...
         <BaseButton variant="outline" @click="addImageSlot">
            Start Adding Photos
         </BaseButton>
      </div>
    </div>
    ...
    <div class="p-8 border-t border-gray-50 bg-slate-50/50 flex gap-4 justify-end">
      <BaseButton variant="outline" class="min-w-[140px]" @click="close">Discard Changes</BaseButton>
      <BaseButton :loading="isSaving" class="min-w-[140px]" @click="save">Save Gallery</BaseButton>
    </div>
  </VueFinalModal>
</template>
