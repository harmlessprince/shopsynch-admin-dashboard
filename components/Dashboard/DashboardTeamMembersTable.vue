    ...
    <table class="w-full text-[1.4rem]">
      <thead>
        <tr class="border-b border-[#E0E0E0]">
          <th class="text-left py-[1.2rem] pr-[2.4rem] font-[600] text-[#616161]">Name</th>
      ...
      <tbody>
        <tr v-for="member in members" :key="member.id" class="border-b border-[#F5F5F5] hover:bg-[#fbfbfb] transition-colors">
          <td class="py-[1.4rem] pr-[2.4rem] font-[500] text-[#1B1B19]">{{ member.fullName }}</td>
          <td class="py-[1.4rem] pr-[2.4rem] text-[#616161]">{{ member.email }}</td>
          <td class="py-[1.4rem] pr-[2.4rem]">
            <DashboardRoleBadge :role-name="member.role?.name ?? '—'" />
          </td>
  ...
  <div class="lg:hidden flex flex-col gap-y-[1.2rem]">
    ...
    >
      <div class="flex flex-col gap-y-[0.4rem]">
        <span class="font-[600] text-[1.4rem] text-[#1B1B19]">{{ member.fullName }}</span>
        <span class="text-[1.2rem] text-[#616161]">{{ member.email }}</span>
        <DashboardRoleBadge :role-name="member.role?.name ?? '—'" class="mt-[0.4rem]" />
      </div>
      <button
...
const emit = defineEmits(['change-role', 'remove'])
...
function emitAction(action) {
  const member = props.members.find(m => m.id === openMenuId.value)
  if (member) emit(action, member)
  openMenuId.value = null
}
